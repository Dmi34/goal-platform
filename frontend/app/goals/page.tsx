'use client'

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Select } from "@/components/ui/select"
import Link from "next/link"
import { GoalCard } from "../components/goal-card"
import { Navbar } from "../components/navbar"
import { Search } from 'lucide-react'
import { FormEvent } from "react"

const goals = [
  {
    id: 1,
    title: "Launch a Successful Tech Startup",
    description: "From idea to IPO: A comprehensive guide to building a unicorn in Silicon Valley.",
    image: "/placeholder.svg?height=100&width=200",
    price: 299.99,
    rating: 4.8,
    author: "Elon Musk Jr.",
    category: "Business",
    status: "popular",
  },
  {
    id: 2,
    title: "Master the Art of Sourdough Bread",
    description: "Become a home bakery sensation with artisanal bread techniques.",
    image: "/placeholder.svg?height=100&width=200",
    price: 49.99,
    rating: 4.9,
    author: "Chef Doughy",
    category: "Culinary",
    status: "new",
  },
  {
    id: 3,
    title: "Run a Marathon in Under 4 Hours",
    description: "Transform from couch potato to marathon finisher with this 6-month training plan.",
    image: "/placeholder.svg?height=100&width=200",
    price: 79.99,
    rating: 4.7,
    author: "Usain Bolt's Cousin",
    category: "Fitness",
    status: "popular",
  },
  {
    id: 4,
    title: "Write and Publish Your First Novel",
    description: "From outline to bestseller: Navigate the world of creative writing and publishing.",
    image: "/placeholder.svg?height=100&width=200",
    price: 129.99,
    rating: 4.6,
    author: "J.K. Rowling's Neighbor",
    category: "Arts",
    status: "new",
  },
  {
    id: 5,
    title: "Learn to Code and Land a Tech Job",
    description: "Zero to hero: Master programming and ace your coding interviews.",
    image: "/placeholder.svg?height=100&width=200",
    price: 199.99,
    rating: 4.9,
    author: "Ex-Googler",
    category: "Technology",
    status: "popular",
  },
]

export default function GoalsCatalog() {
  const handleSearch = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    // Implement search functionality here
    console.log("Search submitted")
  }

  return (
    <div className="min-h-screen bg-[#0D0D0D] text-slate-100">
      <Navbar />
      <main className="container mx-auto px-4 py-8">
        <h1 className="mb-8 text-3xl font-bold">Discover Goals</h1>
        <form onSubmit={handleSearch} className="mb-8 flex flex-wrap gap-4">
          <div className="flex-1 min-w-[200px] relative">
            <Input
              type="search"
              placeholder="Search goals..."
              className="w-full bg-neutral-800 text-slate-100 pr-10"
            />
            <Button 
              type="submit" 
              size="icon"
              className="absolute right-0 top-0 bg-red-600 hover:bg-red-700"
            >
              <Search className="h-4 w-4" />
            </Button>
          </div>
          <Select defaultValue="all" className="max-w-xs bg-neutral-800 text-slate-100">
            <option value="all">All Categories</option>
            <option value="business">Business</option>
            <option value="culinary">Culinary</option>
            <option value="fitness">Fitness</option>
            <option value="arts">Arts</option>
            <option value="technology">Technology</option>
          </Select>
          <Select defaultValue="all" className="max-w-xs bg-neutral-800 text-slate-100">
            <option value="all">All Prices</option>
            <option value="free">Free</option>
            <option value="paid">Paid</option>
          </Select>
          <Select defaultValue="all" className="max-w-xs bg-neutral-800 text-slate-100">
            <option value="all">All Statuses</option>
            <option value="new">New</option>
            <option value="popular">Popular</option>
          </Select>
          <Select defaultValue="relevance" className="max-w-xs bg-neutral-800 text-slate-100">
            <option value="relevance">Sort by Relevance</option>
            <option value="price-low">Price: Low to High</option>
            <option value="price-high">Price: High to Low</option>
            <option value="rating">Highest Rated</option>
          </Select>
        </form>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {goals.map((goal) => (
            <GoalCard key={goal.id} goal={goal} />
          ))}
        </div>
      </main>
    </div>
  )
}

